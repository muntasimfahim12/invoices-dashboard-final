...
export default function ProjectsPage() {
    ...
    const fetchData = useCallback(async (isSilent = false) => {
        ...
        try {
            ...
            // আপনার ব্যাকএন্ড লজিক অনুযায়ী 'clinets' এন্ডপয়েন্ট থেকে ডাটা ফেচ করা
                ...
                response.data.forEach((client: any) => {
                    if (client.projects && Array.isArray(client.projects)) {
                        client.projects.forEach((prj: any) => {
                            projectsList.push({
                                ...prj,
                                // রিলেশনাল ডাটা হ্যান্ডলিং
                                internalId: prj._id || `${client._id}-${prj.name}`,
                                clientId: client._id,
            ...
            // লেটেস্ট প্রজেক্টগুলো আগে দেখানোর জন্য সর্টিং
            setAllProjects(projectsList.sort((a, b) => 
    ...
    const stats = useMemo(() => {
        ...
        const active = allProjects.filter(p => p.status?.toLowerCase() === 'active').length;
    ...
    const filteredProjects = useMemo(() => {
        return allProjects.filter(prj => {
            ...
            const matchesStatus = statusFilter === "All" || 
                prj.status?.toLowerCase() === statusFilter.toLowerCase();
    ...
    // --- DELETE LOGIC (Backend Sync) ---
    const handleDelete = async (clientId: string, projectId: string, internalId: string) => {
        if (!window.confirm(`Are you sure you want to delete this project?`)) return;
        try {
            setActionLoading(`deleting-${internalId}`);
            // ব্যাকএন্ডে ক্লায়েন্ট অবজেক্ট থেকে প্রজেক্ট রিমুভ করা
            const clientRes = await axios.get(`${API_BASE}/clinets/${clientId}`);
            const updatedProjects = clientRes.data.projects.filter((p: any) => p._id !== projectId);
    ...
    // --- UPDATE LOGIC (Backend Sync) ---
    const handleEditSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        setActionLoading('updating');
        try {
            const { clientId, _id, name, budget, status } = selectedProject;
            const clientRes = await axios.get(`${API_BASE}/clinets/${clientId}`);
            
            // স্পেসিফিক প্রজেক্ট আপডেট লজিক
            const updatedProjects = clientRes.data.projects.map((p: any) =>
                p._id === _id ? { ...p, name, budget: Number(budget), status } : p
            );
            
            await axios.put(`${API_BASE}/clinets/${clientId}`, { projects: updatedProjects });
            
            setIsEditModalOpen(false);
            await fetchData(true); // সাইলেন্ট রিফ্রেশ
        } catch (error) { 
            alert("Update failed."); 
        } finally { 
            setActionLoading(null); 
    ...
    return (
        <div className="min-h-screen bg-[#FFFFFF] p-4 lg:p-8 selection:bg-[#4177BC] selection:text-white font-sans text-slate-900">
            <div className="max-w-400 mx-auto">
                ...
                <nav className="flex items-center justify-between mb-8 bg-white/80 backdrop-blur-xl p-4 rounded-4xl sticky top-4 z-60 border border-white shadow-sm">
                    <Link href="/admin">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <main className="lg:col-span-9 space-y-8">
                        ...
                        <div className="bg-white rounded-[40px] border border-slate-100 shadow-2xl shadow-slate-200/30 overflow-hidden">
                            <div className="p-6 md:p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-center gap-6 bg-linear-to-r from-white to-slate-50/50">
                                                    ...
                                                    >
                                                        <td className="px-8 py-6">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <div>
                                                                    ...
                                                                    <p className="text-[10px] text-slate-400 font-medium">ID: {item.internalId.toString().slice(-6).toUpperCase()}</p>
                                                                </div>
                                                            </div>
                                                        </td>
                                                        <td className="px-8 py-6">
                                                        ...
                                                        <td className="px-8 py-6 text-right">
                                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                                                <button onClick={() => { setSelectedProject({ ...item }); setIsEditModalOpen(true); }} className="p-2.5 bg-white shadow-sm border border-slate-100 text-slate-400 hover:text-[#4177BC] hover:border-blue-200 rounded-xl transition-all"><Edit3 size={16} /></button>
                                                                <button onClick={() => handleDelete(item.clientId, item._id, item.internalId)} className="p-2.5 bg-white shadow-sm border border-slate-100 text-slate-400 hover:text-red-600 hover:border-red-100 rounded-xl transition-all">
                            ...
                            {/* --- PAGINATION (Functionality Updated) --- */}
                            <div className="p-6 md:p-8 border-t border-slate-50 flex flex-col sm:flex-row items-center justify-between gap-6 bg-slate-50/30">
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest order-2 sm:order-1">
                                    Displaying <span className="text-slate-900">{filteredProjects.length > 0 ? startIndex + 1 : 0} - {Math.min(startIndex + itemsPerPage, filteredProjects.length)}</span> of {filteredProjects.length} Architectural Records
                                </p>
                                ...
                                <div className="flex items-center gap-3 order-1 sm:order-2">
                                    <button
                                        ...
                                        className="p-3 rounded-2xl border border-slate-200 bg-white text-slate-400 disabled:opacity-30 hover:bg-[#4177BC] hover:text-white transition-all shadow-sm"
                                    >
                                        <ChevronLeft size={18} />
                                    </button>
                                    ...
                                    <div className="flex items-center gap-2">
                                        {[...Array(totalPages)].map((_, i) => (
                                            <button
                                                ...
                                                className={`w-10 h-10 rounded-2xl text-[10px] font-black transition-all ${currentPage === i + 1
                                                        ? "bg-[#4177BC] text-white shadow-xl scale-110"
                                                        : "bg-white text-slate-400 border border-slate-200 hover:text-[#4177BC]"
                                                    }`}
                                            >
                                                {i + 1}
                                            </button>
                        ...
                        >
                            ...
                            <div className="flex justify-between items-center mb-10 relative z-10">
                                <div>
                                    ...
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Record ID: {selectedProject?.internalId?.toString().slice(-8)}</p>
                                </div>
...
// --- SUB-COMPONENTS (Styles Kept 100% Same) ---
