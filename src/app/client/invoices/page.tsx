...
export default function ClientInvoiceList() {
    ...
    // ✅ FIX: status state add করা হয়েছে (কোনো লাইন কাটা হয়নি)
    ...
    const totalDue = invoices.reduce(
        (acc, inv) => acc + (Number(inv.remainingDue) || 0),
        0
    );
    ...
    if (loading)
        return (
            <div className="p-10 text-center font-black uppercase tracking-widest text-xs animate-pulse">
                Loading Portal...
            </div>
        );
        ...
        <div className="min-h-screen bg-[#F8FAFC] p-6 md:p-12">
            <div className="max-w-7xl mx-auto space-y-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <h1 className="text-3xl font-[1000] text-slate-900 tracking-tighter">
                            My Invoices
                        </h1>
                        <p className="text-slate-500 font-medium">
                            Manage and track your project billing history
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredInvoices.map((inv) => (
                        <div
                            key={inv._id}
                            className="group bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm hover:shadow-xl hover:shadow-blue-500/5 transition-all relative overflow-hidden"
                        >
                            {/* Status Badge */}
                            ...
                            <h3 className="text-lg font-black text-slate-900 mb-1 group-hover:text-[#4177BC] transition-colors">
                                {inv.projectTitle || "Project Services"}
                            </h3>
                            <p className="text-xs font-bold text-slate-400 mb-6 uppercase tracking-tighter">
                                ID: {inv.invoiceId}
                            </p>
                            ...
                            <FileText
                                className="absolute -bottom-4 -right-4 text-slate-50 opacity-10 group-hover:opacity-20 transition-opacity"
                                size={120}
                            />
                        </div>
                    ))}
                ...
                {filteredInvoices.length === 0 && (
                    <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-slate-200">
                        ...
                        <h3 className="font-black text-slate-400 uppercase text-xs tracking-[0.2em]">
                            No invoices found
                        </h3>
                    </div>
                )}
